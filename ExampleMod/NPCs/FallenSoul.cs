...
namespace ExampleMod.NPCs
{
	...
	// It implements the `CustomBehavior` and `ShouldMove` virtual methods being overridden here.
	public class FallenSoul : Hover
	{
		...
		// Only show name and health of the NPC when close to the player.
		public override bool ShowHoverInfo()
		{
			return npc.Distance(Main.player[npc.target].Center) < 200;
		}
	...
	public class PurificationPowder : GlobalProjectile
	{
		...
		public override void PostDamage(Projectile projectile, Rectangle projHitbox)
		{
			...
			{
				if (Main.npc[i].active && Main.npc[i].type == NPCID.Wraith && projHitbox.Intersects(Main.npc[i].Hitbox))
				{
