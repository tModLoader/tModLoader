...
public class ExampleModConfig : ModConfig
{
	...
	[Tooltip(
		"$Mods.ExampleMod.Config.ExampleWingsToggle.Tooltip")] // A tooltip is a description showed when you hover your mouse over the option. It can be used as a more in-depth explanation of the option.
	[DefaultValue(true)] // This sets the configs default value.
	[ReloadRequired]
	// Marking it with [ReloadRequired] makes tModLoader force a mod reload if the option is changed. It should be used for things like item toggles, which only take effect during mod loading
	public bool ExampleWingsToggle; // To see the implementation of this option, see ExampleWings.cs

	// TODO: More detailed professional English description
	// Decide whether to load DynamicModConfig
	// Loading order: Config, Localization, ILoadable(System/Player.etc), Mod.Load
	[DefaultValue(false)]
	[ReloadRequired]
	public bool DynamicConfig;

	public override void OnLoaded() {
		if (DynamicConfig && DynamicModConfig.Instance == null) {
			var type = typeof(DynamicModConfig);
