...
namespace ExampleMod.Common.GlobalNPCs
{
	class ExampleNPCShop : GlobalNPC
	{
		public override void ModifyShop(NPCShop shop) {
			if (shop.FullName == TMLLootDatabase.GetNPCShopName(NPCID.Dryad)) {
			...
			else if (shop.FullName == TMLLootDatabase.GetNPCShopName(NPCID.Wizard)) {
				// shopContents.Add(ModContent.ItemType<Infinity>(), ChestLoot.Condition.InExpertMode);
			}
			else if (shop.FullName == TMLLootDatabase.GetNPCShopName(NPCID.Stylist)) {
