...
namespace ExampleMod.Content.Projectiles
{
	public class ExampleCustomSwingProjectile : ModProjectile
	{
		...
		public ref float timer => ref Projectile.ai[1]; // Timer to keep track of progression of each stage
		public ref float targetAngle => ref Projectile.ai[2]; // Angle aimed in (with constraints)
		...
		public override void SetDefaults() {
			...
			Projectile.localNPCHitCooldown = 1000; // We set this to an high number to make sure the projectile doesn't hit twice
		...
		public override void OnSpawn(IEntitySource source) {
			...
			targetAngle = (float)Math.Atan2(Main.MouseWorld.Y - player.MountedCenter.Y, Main.MouseWorld.X - player.MountedCenter.X);
		...
		public override void AI() {
			...
			float prepTime = 12f / Main.player[Projectile.owner].GetTotalAttackSpeed<MeleeDamageClass>();
			float execTime = 12f / Main.player[Projectile.owner].GetTotalAttackSpeed<MeleeDamageClass>();
			float hideTime = 12f / Main.player[Projectile.owner].GetTotalAttackSpeed<MeleeDamageClass>();
		...
		public void setSwordPosition(float rotation, float size = 1) {
			...
			player.heldProj = Projectile.whoAmI; // set held projectile to this projectile
			...
			player.SetCompositeArmFront(true, CompositeArmStretchAmount.Full, rotation - MathHelper.ToRadians(90f)); // set arm position (90 degree offset since arm starts lowered)
			Vector2 armPosition = player.GetFrontHandPosition(CompositeArmStretchAmount.Full, rotation - (float)Math.PI / 2); // get position of hand
			...
			Projectile.scale = size * 1.2f * player.GetAdjustedItemScale(player.HeldItem); // Slightly scale up the projectile and also take into account melee size modifiers
