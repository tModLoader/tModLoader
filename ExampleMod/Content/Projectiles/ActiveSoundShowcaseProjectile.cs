...
namespace ExampleMod.Content.Projectiles
{
	...
	public class ActiveSoundShowcaseProjectile : ModProjectile
	{
		...
		SoundStyle soundStyleTwister = new SoundStyle("Terraria/Sounds/Custom/dd2_book_staff_twister_loop") {
			PauseWithGame = true, // Rather than keep playing, this sound pauses when the game is paused.
		};

		SoundStyle soundStyleIgniteLoop = new SoundStyle("Terraria/Sounds/Custom/dd2_kobold_ignite_loop") {
			IsLooped = true,
			StopWhenGamePaused = true, // Note that the code examples using this SoundStyle automatically start this sound once more when the game is un-paused.
		...
		public override void AI() {
			...
			// By default (in single player) sounds continue playing when the game is paused or loses focus (player switches to another program). In some situations the modder might want to pause and resume a sound when the game pauses and resumes, in other situations the modder might just want to stop the sound when the game pauses. These behaviors can be implemented by setting PauseWithGame or StopWhenGamePaused on the SoundStyle. The 2 SoundStyles in this example use each of these options because the sound clips are quite long.
