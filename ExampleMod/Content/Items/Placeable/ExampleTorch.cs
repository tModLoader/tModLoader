...
namespace ExampleMod.Content.Items.Placeable
{
	public class ExampleTorch : ModItem
	{
		...
		public override void SetDefaults() {
			...
			Item.createTile = ModContent.GetId<Tiles.ExampleTorch>();
			Item.width = 10;
			Item.height = 12;
			Item.value = 50;
		}
		...
		public override void HoldItem(Player player) {
			...
			if (Main.rand.Next(player.itemAnimation > 0 ? 40 : 80) == 0) {
				Dust.NewDust(new Vector2(player.itemLocation.X + 16f * player.direction, player.itemLocation.Y - 14f * player.gravDir), 4, 4, ModContent.GetId<Sparkle>());
			}
