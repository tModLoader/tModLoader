...
namespace Content.Items.Accessories
{
    ...
    {
        // We override this method to set default properties of our accessory 
        public override void SetDefaults()
        {
            Item.width = 30; // We set width of entity hitbox to 30
            Item.height = 32; // We set height of entity hitbox to 32
            Item.accessory = true; // We set that our item is an accesory
            Item.value = Item.sellPrice(gold: 10); // We set sell price for our accessory to 10 gold
            Item.rare = ItemRarityID.Green; // We set rarity of this item to Green
        }
        // Creates a recipe for ExclusiveAccessory
        // You need one Sun Stone and you make it at any anvil
        public override void AddRecipes()
        {
            Recipe recipe = CreateRecipe()
            .AddIngredient(ItemID.SunStone, 1) // Ingredients we use to make it (Type of Item, Amount of Item)
            .AddTile(TileID.Anvils) // Tile at which we craft it
            .Register();
        }
        ...
        public override void RightClick(Player player)
        {
            ...
            if (accessory != null)
            {
                Main.LocalPlayer.QuickSpawnClonedItemDirect(player.GetSource_FromThis(), accessory);
    ...
    public class GreenExclusiveAccessory : ExclusiveAccessory
    {
        ...
        public override void ModifyTooltips(List<TooltipLine> tooltips)
        {
            tooltips.Add(new TooltipLine(Mod, 
                "Damage", 
                "Increases Melee Damage by 50%. Increases Ranged Damage by 50%")); // Adding a line in a tooltip that tells what it does
            base.ModifyTooltips(tooltips); // Calling base method
        }
        public override void RightClick(Player player)
        {
            ...
            SoundEngine.PlaySound(SoundID.MaxMana, new Vector2((int)player.position.X, (int)player.position.Y));
        }
    }
    ...
    public class YellowExclusiveAccessory : ExclusiveAccessory
    {
        ...
        // Here we can place code for special effects of our accessory
        public override void UpdateAccessory(Player player, bool hideVisual)
        {
            if (Main.dayTime)
            {
                // 100% melee damage increase
                player.GetDamage(DamageClass.Melee) += 1f;
            }
            else
            {
                // 100% ranged damage increase
        ...
        public override void ModifyTooltips(List<TooltipLine> tooltips)
        {
            tooltips.Add(new TooltipLine(Mod, 
                "Damage", 
                "Increases Melee Damage by 100% at day time. Increases Ranged Damage by 100% at night time")); // Adding a line in a tooltip that tells what it does
            base.ModifyTooltips(tooltips); // Calling base method
        }
        public override void RightClick(Player player)
        {
