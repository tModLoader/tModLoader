...
* World generation now properly reports errors to the player. Previously, if a `GenPass` (including modded ones) threw an exception, it was silently logged and world generation would continue, potentially saving a broken world. Now the exception propagates, an error message is shown, and the world is not saved.