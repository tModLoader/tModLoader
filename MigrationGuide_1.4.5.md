...
* `ModSystem.ModifyCreativePowersMenu(CreativePowerMenuEntries entries)` has been added to allow mods to add controls to Journey Mode's powers menu.
  * Use `entries.AddToggle(...)` to add a top-level toggle button directly to the main powers strip.
  * Use `entries.AddCategory(...)` to add a custom parent category, then call `AddButton(...)` or `AddSlider(...)` on the returned `CreativePowerMenuCategory` to add child controls.
  * Localization keys passed to these helpers should include the appropriate suffixes. Toggles use `.Enabled`, `.Disabled`, and optionally `.Description`; categories and sliders use `.Opened`, `.Closed`, and optionally `.Description`; sliders can also use `.Top`, `.Middle`, and `.Bottom` for default labels.
  * See `ExampleCreativePowersMenuSystem` in Example Mod for a complete example.
