...
* Mods can now add custom world creation difficulty and evil biome options without IL editing or `On` hooks by overriding `ModSystem.ModifyWorldCreationMenuOptions(ref List<WorldCreationMenuOption> difficultyOptions, ref List<WorldCreationMenuOption> evilOptions)`.
  * Add `WorldCreationMenuOption` entries to the supplied lists. Each option has a stable `Id`, display text, description, text color, button icon path, selection callbacks, and optional preview texture paths.
  * Modded difficulty option IDs are assigned automatically starting at `GameModeID.TML`; modded evil biome option IDs are assigned automatically starting at `WorldEvilID.TML`. Use the option's assigned `WorldCreationMenuOption.Type` in the option's `OnSelected` callback instead of reusing vanilla IDs such as `GameModeID.Expert` or `WorldEvilID.Crimson` when the option represents truly custom behavior.
  * `GameModeID.Count` and `WorldEvilID.Count` are updated after all mods modify the option lists so custom IDs from multiple mods do not conflict.
  * Use `WorldCreationMenuOption.PreviewValue` for vanilla-style preview art. Difficulty preview values are `GameModeID.Normal`, `GameModeID.Expert`, `GameModeID.Master`, and `GameModeID.Creative`; evil preview values are `0` for random, `1` for corruption, and `2` for crimson.
  * Use `byte.MaxValue` as `PreviewValue` for a fully custom preview that should not draw the corresponding vanilla difficulty or evil preview layer.
  * `PreviewTexturePath` and `PreviewTexturePath2` can be used for custom world creation preview artwork. Custom preview textures should be 76x76, matching vanilla world creation preview layers. For difficulty options, `PreviewTexturePath` is the first layer and `PreviewTexturePath2` is the second layer. For evil biome options, use `PreviewTexturePath`.
  * `IconTexturePath` supports vanilla asset paths such as `Images/UI/WorldCreation/IconDifficultyExpert` and mod asset paths such as `ExampleMod/Assets/Textures/WorldCreation/IconDifficultyExample`.
  * Store custom selections in `SaveWorldHeader` if they should be visible from the world select menu. Use `ModifyWorldListDisplay` to show custom difficulty text, colors, and world icons. If a custom world list icon must be visible immediately when opening the world select menu, request it with `AssetRequestMode.ImmediateLoad`.
