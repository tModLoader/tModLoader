...
namespace Terraria.ModLoader
{
	...
	public static class ItemLoader
	{
		...
		private static readonly HookList HookOnConsumeAmmo = AddHook<Action<Item, Player>>(g => g.OnConsumeAmmo);
		/// <summary>
		...
		internal static void VerifyGlobalItem(GlobalItem item) {
			...
			int saveMethods = 0;
			if (HasMethod(type, "NeedsSaving", typeof(Item))) {
				saveMethods++;
			}

			if (HasMethod(type, "Save", typeof(Item))) {
				saveMethods++;
			}

			if (HasMethod(type, "Load", typeof(Item), typeof(TagCompound))) {
				saveMethods++;
			}
			...
			int netMethods = 0;
			if (HasMethod(type, "NetSend", typeof(Item), typeof(BinaryWriter))) {
				netMethods++;
			}

			if (HasMethod(type, "NetReceive", typeof(Item), typeof(BinaryReader))) {
				netMethods++;
			}
