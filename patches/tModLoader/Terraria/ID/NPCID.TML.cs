...
public partial class NPCID
{
	public static partial class Sets
	{
		...
		/// Use this for NPC that can be attacked that should be immune to all normal debuffs. Tag debuffs are special debuffs that facilitate combat mechanics, such as the "summon tag damage" applied by whip weapons. Wraith, Reaper, CultistBoss, the Celestial Pillars, The Destroyer, and the Martian Saucer Turret/Cannon/Core are examples of NPC that use this setting.<br/><br/>
		/// Modders can specify specific buffs to be vulnerable to by assigning <see cref="SpecificDebuffImmunity"/> to false.
		/// </summary>
		...
		/// By default, NPC aren't immune to any buffs, but <see cref="DefaultDebuffImmunity"/> or <see cref="FullyImmuneToBuffs"/> can make an NPC immune to all buffs. The values in this set override those settings.<br/><br/>
		...
		static Sets()
		{
			FullyImmuneToBuffs = Factory.CreateBoolSet();
