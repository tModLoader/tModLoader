...
namespace Terraria.ModLoader
{
	...
	public static class ModContent
	{
		...
		internal static void Load(CancellationToken token) {
			...
			DetectModChangesForInfoMessage();

			MenuLoader.GotoSavedModMenu();
			BossBarLoader.GotoSavedStyle();
		}

		private static void DetectModChangesForInfoMessage() {
			if (ModLoader.DetectedModChangesForInfoMessage) // Do not do anything if it's not the first load after launch
				return;

			ModLoader.DetectedModChangesForInfoMessage = true;

			var currMods = ModOrganizer.GetFormattedDisplayNameAndVersionByMod();
			...
			if (ModLoader.showNewUpdatedModsInfo && File.Exists(fileName)) {
				...
				try {
					...
					// If any info is accumulated, display it (using OnSuccessfulLoad because otherwise end of mod load sets menuMode to 0 without showing the info)
					// TODO find a better place for this post-mod load: how compatible is it to pass 0 (main menu) for gotoMenu if other sources also utilize this?
					if (messages.Length > 0)
						ModLoader.OnSuccessfulLoad += () => Interface.infoMessage.Show(Language.GetTextValue("tModLoader.ShowNewUpdatedModsInfoMessage") + messages.ToString(), 0); //TODO message mention that it can be disabled config
				}
				catch {
