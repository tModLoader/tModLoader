...
public abstract class ModBlockType : ModTexturedType, ILocalizedModType
{
	...
	/// <summary>
	/// The type of item dropped when this tile/wall is killed. Defaults to 0, which means no item specified.
	...
	/// <br/> For ModTile, use <see cref="ModTile.RegisterItemDrop(int, int)"/> to manually register a drop for a specific tile style or use <see cref="ModTile.GetItemDrops"/> to fully control item drops depending on tile style or other conditions.
	...
	/// </summary>
	public int ItemDropOverride { get; set; }
