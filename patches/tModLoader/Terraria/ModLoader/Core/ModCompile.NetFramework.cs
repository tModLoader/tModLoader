...
namespace Terraria.ModLoader.Core
{
	...
	internal class ModCompile
	{
		...
		private static bool mono { get; } = Type.GetType("Mono.Runtime") != null;

		internal static readonly Version minDotNetVersion = new Version(4, 6);
		internal static readonly Version minMonoVersion = new Version(5, 20);
		internal static bool RoslynCompatibleFrameworkCheck(out string msg) {
			...
			if (!mono && FrameworkVersion.Version >= minDotNetVersion ||
				mono && FrameworkVersion.Version >= minMonoVersion) {

				msg = Language.GetTextValue("tModLoader.DMDotNetSatisfied", $"{(mono ? "Mono" : ".NET Framework")} {FrameworkVersion.Version}");
				return true;
			}
			...
			try {
				...
				using (var asmResolver = new BuildAssemblyResolver(xna)) {
					...
					asm.Write(tempDllPath, new WriterParameters {
						WriteSymbols = true,
						SymbolWriterProvider = !mono ? new NativePdbWriterProvider() : null
					});
