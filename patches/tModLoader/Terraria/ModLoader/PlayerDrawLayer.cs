...
/// <br/><br/> Custom PlayerDrawLayer can be used to add additional layers to player drawing. There are a few options for customizing player rendering without resorting to a custom PlayerDrawLayer for some specific use-cases: <see cref="ModItem.ModifyEquipTextureDraw(ref PlayerDrawSet, ref DrawData, Terraria.ModLoader.EquipType, int, string)"/> can be used to modify how an equipment texture for an item is drawn. <see cref="ModItem.ModifyItemDraw(ref PlayerDrawSet, ref DrawData, ref DrawData?, ref DrawData?)"/> can be used to modify how the held item is drawn.
/// </summary>
[Autoload]
