...
namespace Terraria.ModLoader.IO
{
	...
	public class TagCompound : IEnumerable<KeyValuePair<string, object>>, ICloneable
	{
		...
		public bool TryGet<T>(string key, out T value) {
			bool hasValue = dict.TryGetValue(key, out object tag);
			try {
		...
		public void Set(string key, object value, bool replace = false) {
			...
			if (replace) dict[key] = serialized;
			else dict.Add(key, serialized);
		}
		...
		public object Clone() {
			...
			foreach (KeyValuePair<string, object> entry in this) copy.Set(entry.Key, TagIO.Clone(entry.Value));
