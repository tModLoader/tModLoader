...
public static class ModNet
{
	...
	private static ReloadRequiredExplanation MakeDownloadModExplanation(LocalMod[] modFiles, ModHeader header, Mod clientMod)
	{
		...
		int subOrder = workshopSearchReturnState switch {
			WorkshopHelper.WorkshopSearchReturnState.NotFound => 1,
			WorkshopHelper.WorkshopSearchReturnState.Success when workshopMod.Downloads < 1000 => 2,
			WorkshopHelper.WorkshopSearchReturnState.Success => 3,
			_ => 4
		};
		...
		if (modFiles.FirstOrDefault(mod => header.MatchesNameAndVersion(mod.modFile)) is LocalMod localMod) {
			...
			//else
			return new ReloadRequiredExplanation(1, header.name, localMod, Language.GetTextValue("tModLoader.ReloadRequiredExplanationDownloadHashDiffers", "00BFFF", header.version, string.Concat(header.hash[..4].Select(b => b.ToString("x2")))), subOrder: subOrder);
		}
		else {
			...
			if (localModMatchingNameOnly == null) {
				...
				return new ReloadRequiredExplanation(1, header.name, null, Language.GetTextValue("tModLoader.ReloadRequiredExplanationDownload", "00BFFF", header.version), subOrder: subOrder);
			}
			else {
				if (clientMod != null) {
					...
					if (clientMod.Version > header.version) {
						...
						return new ReloadRequiredExplanation(1, header.name, localModMatchingNameOnly, Language.GetTextValue(downgradeIsTemporary ? "tModLoader.ReloadRequiredExplanationDownloadDowngradeTemporary" : "tModLoader.ReloadRequiredExplanationDownloadDowngrade", "00BFFF", header.version, clientMod.Version), subOrder: subOrder);
					}
					else {
						return new ReloadRequiredExplanation(1, header.name, localModMatchingNameOnly, Language.GetTextValue("tModLoader.ReloadRequiredExplanationDownloadUpgrade", "00BFFF", header.version, clientMod.Version), subOrder: subOrder);
					}
				}
				else {
					// We have the mod, but not the correct version.
					return new ReloadRequiredExplanation(1, header.name, localModMatchingNameOnly, Language.GetTextValue("tModLoader.ReloadRequiredExplanationDownload", "00BFFF", header.version), subOrder: subOrder);
				}
			}
		}
	}
