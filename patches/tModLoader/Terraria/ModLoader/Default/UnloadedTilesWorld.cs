	...
	class UnloadedTilesWorld : ModWorld
	{
		...
		public override void Load(TagCompound tag) {
			...
			foreach (var infoTag in tag.GetList<TagCompound>("list")) {
				...
				int type = ModContent.TryGet(modName, name, out ModTile tile) ? tile.Type : 0;
		...
		private void RestoreTiles(List<ushort> canRestore) {
			ushort unloadedType = ModContent.Get<ModTile>("ModLoader/UnloadedTile").Type;
		...
		private void ConfirmPendingInfo() {
			...
			ushort pendingType = ModContent.Get<ModTile>("ModLoader/PendingUnloadedTile").Type;
			ushort unloadedType = ModContent.Get<ModTile>("ModLoader/UnloadedTile").Type;
			for (int x = 0; x < Main.maxTilesX; x++) {
				for (int y = 0; y < Main.maxTilesY; y++) {
