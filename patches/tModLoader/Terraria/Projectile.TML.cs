...
#region Globals
	...
	public bool CanShimmer()
		=> ProjectileLoader.CanShimmer(this) && ShimmerLoader<Projectile>.AnyValidModShimmer(this);

	Vector2 IModShimmerable.Velocity { get => velocity; set => velocity = value; }
	int IModShimmerable.ShimmerRedirectedType => ShimmerLoader<Projectile>.GetRedirectedType(type);
	...
	private void ModdedProjectileShimmer()
	{
		...
		if (CanShimmer()) { // If can shimmer
			...
			if (Main.netMode != NetmodeID.MultiplayerClient && shimmerTransformTime > 0.9f && !PreventingChainedShimmers) {
				...
				ShimmerLoader<Projectile>.TryModShimmer(this);
				OnShimmer();
			}
