...
public sealed record Condition(LocalizedText Description, Func<bool> Predicate)
{
	...
	// Parameters
	public static Condition PlayerCarriesItem([IDType(IDTypeAttribute.Item)] int itemId) => new(Language.GetText("Conditions.PlayerCarriesItem").WithFormatArgs(Lang.GetItemName(itemId)), () => Main.LocalPlayer.HasItem(itemId));
	...
	public static Condition NpcIsPresent([IDType(IDTypeAttribute.NPC)] int npcId) => new(Language.GetText("Conditions.NpcIsPresent").WithFormatArgs(Lang.GetNPCName(npcId)), () => NPC.AnyNPCs(npcId));
