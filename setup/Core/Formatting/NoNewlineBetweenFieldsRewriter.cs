...
namespace Terraria.ModLoader.Setup.Core.Formatting
{
	...
	{
		private readonly HashSet<SyntaxToken> modifyTokens = new();
		...
		private void TagFieldTokens(SyntaxList<MemberDeclarationSyntax> members)
		{
			for (int i = 0; i < members.Count - 1; i++) {
				if ((members[i] is FieldDeclarationSyntax || members[i] is EventFieldDeclarationSyntax) &&
				    members[i].Kind() == members[i + 1].Kind()) {
		...
		{
			if (token.HasLeadingTrivia && token.LeadingTrivia[0].IsKind(SyntaxKind.EndOfLineTrivia) &&
			    token.LeadingTrivia.All(SyntaxUtils.IsWhitespace)) {
