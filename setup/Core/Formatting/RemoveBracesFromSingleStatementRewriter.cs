...
{
	internal class RemoveBracesFromSingleStatementRewriter : CSharpSyntaxRewriter
	{
		...
		private bool NoClausesNeedBraces(IfStatementSyntax ifStmt)
		{
			...
			while (true) {
				...
				StatementSyntax? elseStmt = ifStmt.Else?.Statement;
				if (elseStmt == null) {
					return true;
				}
		...
		private SyntaxToken EnsureEndsLine(SyntaxToken token) =>
			token.WithTrailingTrivia(EnsureEndsLine(token.TrailingTrivia));

		private StatementSyntax EnsureEndsLine(StatementSyntax node) =>
			node.WithTrailingTrivia(EnsureEndsLine(node.GetTrailingTrivia()));

		private SyntaxTriviaList EnsureEndsLine(SyntaxTriviaList trivia) =>
			trivia.LastOrDefault().IsKind(SyntaxKind.EndOfLineTrivia)
				? trivia
